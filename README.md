...
## 快速纠错版
 ...
    - 假设由于种种原因需要手动把JSON文本转换成JSON对象，请使用JSON.stringify()，避免使用eval，如果一定要使用eval()，务必知道自己使用的理由。

  - 操作DOM时，发现问题，首先排查问题代码运行那一时刻，要操作的DOM元素是否已经存在以及存在状态是否符合预期！

  - 变量声明会提升，但是赋值不会提升（两者区别具体参考‘提升’章节）。 为了避免混淆和意外，请在作用域的最开始声明变量，对变量的赋值可以在需要的地方进行！
